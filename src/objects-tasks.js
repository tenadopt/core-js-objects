...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([letter, positionsWord]) => {
    positionsWord.forEach((position) => {
...
class Selector {
  ...
  checkOrder(current) {
    ...
    if (
      this.order.some(
        (o) => orderPriority.indexOf(o) > orderPriority.indexOf(current)
      )
    ) {
      throw new Error(
    ...
    if (
      ['element', 'id', 'pseudoElement'].includes(current) &&
      this.order.includes(current)
    ) {
      throw new Error(
